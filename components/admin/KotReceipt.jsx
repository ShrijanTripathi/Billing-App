...
const KotReceipt = forwardRef(function KotReceipt({ bill = {}, className = "" }, ref) {
  const items = bill?.items || [];
  const total = Number(
    bill?.total ?? bill?.grandTotal ?? bill?.amount ?? bill?.totalAmount ??
      items.reduce((s, it) => s + (Number(it.qty || it.quantity || 0) * Number(it.price || it.rate || 0) || 0), 0)
  );
  const billNo = bill?.billNo || bill?.billNumber || bill?.invoiceNo || bill?.id || "";
  const customerName = bill?.customerName || bill?.customer || bill?.name || "";
  const instructions = bill?.instructions || bill?.notes || bill?.customerDescription || "";
  ...
  return (
    <section ref={ref} className={`kot-receipt-shell ${className}`.trim()} aria-label="Kitchen Order Ticket">
      <article className="kot-receipt">
        ...
        {typeof total === "number" && !Number.isNaN(total) ? (
          <div className="kot-total-row">
            <div className="kot-small-label">TOTAL</div>
            <div className="kot-total-value">{Number(total).toFixed(2)}</div>
          </div>
        ) : null}
      ...
      <style>{`
        ...
        /* Total row */
        .kot-total-row { display: flex; justify-content: space-between; align-items: center; margin: 4px 0; }
        .kot-total-value { font-size: 14px; font-weight: 800; }
      `}</style>
    </section>
  );
});
